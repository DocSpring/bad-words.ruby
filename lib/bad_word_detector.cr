...
class BadWordDetector
    ...
    until queue.empty?
      ...
      if new_states
        ...
        if success_index
          ...
          return new_state.text, new_state.size
        else
  ...
  def get_new_states(state, string)
    next_symbols = get_next_symbols state.size, string
    unless next_symbols
      return nil
    end
  ...
  def get_rules(char, string)
    ...
    string_rules = @string_sets.select do |k, _|
      k.start_with?(char) && string.start_with?(k)
    end.values.flatten
